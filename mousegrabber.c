...
{
    xcb_window_t root = globalconf.protocol_screens.tab[globalconf.default_screen].screen->root;
...
luaA_mousegrabber_run(lua_State *L)
{
    ...
    if(cfont)
    {
        xcb_cursor_t cursor = xcursor_new(globalconf.protocol_screens.tab[globalconf.default_screen].cursor_ctx, cfont);
